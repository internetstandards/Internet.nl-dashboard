...
def test_get_publicly_shared_lists_per_account(db):
    with freeze_time("2020-01-01"):
        ...
        # no content no crash
        response = c.get("/api/v1/public-reports/accounts/1")
        ...
        response = c.get(f"/api/v1/public-reports/accounts/{account.id}")
        ...
        # non existing list:
        response = c.get(f"/api/v1/public-reports/accounts/{account.id}/urllists/781263187")
        assert response.content == b"[]"

        # non existing account:
        response = c.get(f"/api/v1/public-reports/accounts/781263187/urllists/{urllist.id}")
        assert response.content == b"[]"

        response = c.get(f"/api/v1/public-reports/accounts/{account.id}/urllists/{urllist.id}")
        json_data = response.json()
        json_data[0]["reports"][0]["public_report_code"] = ""
        assert json_data == expected_response
        assert response.status_code == 200
