...
def test_suggest_subdomains_http(db, client):
    ...
    assert (
        client.get("/data/urllists/1/url-suggestions", {"domain": "test.nl"}).status_code == 401
    ...
    response = client.get("/data/urllists/1/url-suggestions", {"domain": "test.nl"})
...
def test_keys_match():
    # a, b, c in any object
    assert (
        keys_are_present_in_object(expected_keys=["a", "b", "c"], any_object={"a": 1, "b": 2, "c": 3, "d": 4}) is True
    )
...
def test_retrieve_possible_urls_from_unfiltered_input_speed() -> None:
    ...
    time = timeit.timeit(lambda: clean_urls(domains), number=25)
...
def test_urllists(db, redis_server) -> None:
    ...
    url_got_removed_from_list = delete_url_from_urllist(
        account, list_1.id, Url.objects.all().filter(url="test.nl").first().id
    )
    ...
    renamed = rename_list(
        account=account,
        list_id=list_2.pk,
        new_name="alksdnalksdnlaksdnlasdknasldknaldnalskndnlaksn"
