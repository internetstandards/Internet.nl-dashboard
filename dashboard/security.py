...
def confirm_keys_are_changed():
    if not settings.DEBUG and settings.SECRET_KEY == '_dzlo^9d#ox6!7c9rju@=u8+4^sprqocy3s*l*ejc2yr34@&98':
        raise ValueError('SECRET_KEY contains a debugging value. Set a sane secret key.')

    if not settings.DEBUG and settings.FIELD_ENCRYPTION_KEY == b'JjvHNnFMfEaGd7Y0SAHBRNZYGGpNs7ydEp-ixmKSvkQ=':
        raise ValueError(
